...
// Page Title
$smarty->assign('pagetitle', 'Edit Supported Server');
...
if(!isset($_POST['update']))
{
    ...
    $db = @mysql_connect($config['sql_host'],$config['sql_user'],$config['sql_pass']) or die('<center><b>Error:</b> <i>editsupportedserver.php</i>: Failed to connect to the database!</center>');
    @mysql_select_db($config['sql_db']) or die('<center><b>Error:</b> <i>editsupportedserver.php</i>: Failed to select the database!</center>');

    //
    // Get client info
    //
    $result = @mysql_query("SELECT * FROM cfg WHERE id = '$url_id'") or die('<center><b>Error:</b> <i>editsupportedserver.php:</i> Failed to list client accounts!</center>');
    ...
    // Display HTML Page
    $smarty->display($config['Template'] . '/editsupportedserver.tpl'); 
}
...
elseif(isset($_POST['update']))
{
    ...
    $update_supp_srv = "UPDATE cfg SET 
                          ...
                          is_punkbuster = '$post_is_punkbuster',
    ...
    @mysql_query($update_supp_srv) or die('<center><b>Error:</b> <i>editsupportedserver.php</i>: Failed to update the Supported Server!</center>');
    
    // Redirect to editsupportedserver.php
    header("Location: editsupportedserver.php?id=$url_id&info=updated");
    exit;
}
