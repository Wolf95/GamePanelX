...
$db = @mysql_connect($config['sql_host'],$config['sql_user'],$config['sql_pass']) or die('<center><b>Error:</b> <i>network.php</i>: Failed to connect to the database!</center>');
...
//
$result_icons = @mysql_query("SELECT href,image,image_text,popup_text FROM pages WHERE page='network.php' AND template='$template' ORDER BY sort_order") or die('<center><b>Error:</b> <i>network.php:</i> Failed to get icon order!</center>');
...
// Display HTML Page
$smarty->display($config['Template'] . '/network.tpl'); 
