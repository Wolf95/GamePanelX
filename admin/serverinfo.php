...
{
    die('<center><b>Error:</b> <i>serverinfo.php:</i> Invalid URL Parameters!</center>');
}
...
$db = @mysql_connect($config['sql_host'],$config['sql_user'],$config['sql_pass']) or die('<center><b>Error:</b> <i>serverinfo.php</i>: Failed to connect to the database!</center>');
@mysql_select_db($config['sql_db']) or die('<center><b>Error:</b> <i>serverinfo.php</i>: Failed to select the database!</center>');
...
if(!empty($url_action) && !empty($url_id))
{
    ...
    {
        // Get server type
        $result_type = @mysql_query("SELECT type FROM servers WHERE id = '$url_id'") or die('<center><b>Error:</b> <i>serverinfo.php:</i> Failed to get server type!</center>');
        ...
        {
            die('<center><b>Error:</b> <i>serverinfo.php:</i> Failed to delete the server!</center>');
        }
        else
        {
...
$result = @mysql_query($server_query) or die('<center><b>Error:</b> <i>serverinfo.php:</i> Failed to get game server information!</center>');
...
if(empty($query_name))
{
    die('<center><b>Error:</b> <i>serverinfo.php:</i> No query engine name found for this server!</center>');
}



// Get the default server query timeout from the `configuration` table
$result_timout = @mysql_query("SELECT value FROM configuration WHERE setting='ServerQueryTimeout'") or die('<center><b>Error:</b> <i>serverinfo.php:</i> Failed to query the configuration table!</center>');
...
//
$result_icons = @mysql_query("SELECT href,image,image_text,popup_text FROM pages WHERE page='serverinfo.php' AND template='$template' ORDER BY sort_order") or die('<center><b>Error:</b> <i>serverinfo.php:</i> Failed to get icon order!</center>');
...
{
    // Display HTML Page
    $smarty->display($config['Template'] . '/serverinfo.tpl');
}
