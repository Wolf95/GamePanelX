...
function saveClientServerDetails()
{
    ...
    var srvrStatus    = $('#srver_status').val();
    var srvDesc       = encodeURIComponent($('#srv_description').val());
    var srvSubDom     = $('#srv_subdomain').val();
    var srvDomain     = $('#srv_domainid').val();
    var srvIP         = $('#srv_ip').val();
    var srvPort       = $('#srv_port').val();
    var srvLogFile    = encodeURIComponent($('#srv_log_file').val());
    var srvMaxSlots   = $('#srv_max_slots').val();
    var srvMap        = encodeURIComponent($('#srv_map').val());
    var srvExe        = encodeURIComponent($('#srv_executable').val());
    var srvWorkDir    = encodeURIComponent($('#srv_working_dir').val());
    var srvSetupDir   = encodeURIComponent($('#srv_setup_dir').val());
    var srvRcon       = encodeURIComponent($('#srv_rcon').val());
...
function cmdSaveAdv(srvID)
{
    ...
    $('#cmd_adv :input').each(function(index,element){
        ...
        else thisVal  = encodeURIComponent(thisVal);
...
function installSupportedServer()
{
    ...
    if($('#is_default').is(':checked')) var isDefault   = '1';
    else var isDefault   = '0';
    
    $.ajax({
        type: "GET",
        url: "../include/ajdb.php",
        data: "a=install_supported_server&networkid="+networkID+"&cfgid="+serverType+"&description="+description+"&is_default="+isDefault,
        beforeSend:function(){
            $('#srv_action_result').html('Beginning Installation ...');
        },
        success: function(html){
            if(html == 'success')
            {
...
function createServer()
{
    ...
    // Some servers don't require cmd-line opts, if not, use default port
    /*
    else
    {
        alert("No port found for this server type.\n\nCheck the Default Server Settings for this game and verify the Port is set.");
        return false;
    }
    */
        ...
        success: function(html){
            ...
            else if(html == 'ok')
            {
                ...
                $.ajax({
                    ...
                    success: function(html){
                        if(html == "success_game")
                        {
                            window.location="servers.php?type=game";
                            return false;
                        }
                        else if(html == "success_voip")
                        {
                            window.location="servers.php?type=voip";
                            return false;
                        }
                        else
                        {
...
function confirmSteamUpdate(wording,serverID)
{
    if(serverID == "")
    {
    ...
    var answer = confirm(wording);
    
    if(answer)
    {
        runSteamUpdate(serverID);
    }
}

function runSteamUpdate(serverID)
{
    if(serverID == "")
    {
        alert("No server ID specified!");
    ...
    $.ajax({
        type: "GET",
        url: "../include/ajdb.php",
        data: "a=steam_update&id="+serverID,
        beforeSend:function(){
            $('#info').hide().html('<i>Starting ...</i>').show();
        },
        success: function(html){
            if(html == 'success')
            {
                $('#info').hide().html('Successfully started the update!').fadeIn();
            }
            else
            {
                alert("Oops: "+html);
