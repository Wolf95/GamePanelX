...
// Page Title
$smarty->assign('pagetitle', 'Add Client Account');
...
$db = @mysql_connect($config['sql_host'],$config['sql_user'],$config['sql_pass']) or die('<center><b>Error:</b> <i>addclient.php</i>: Failed to connect to the database!</center>');
@mysql_select_db($config['sql_db']) or die('<center><b>Error:</b> <i>addclient.php</i>: Failed to select the database!</center>');
...
if(!isset($_POST['create']))
{
    ...
    // Display HTML Page
    $smarty->display($config['Template'] . '/addclient.tpl');
}
...
// Create Account
if(!gpx_acct_create_client($post_username,$post_password,$post_status,$post_first_name,$post_middle_name,$post_last_name,$post_company,$post_phone,$post_email,$post_address1,$post_address2,$post_city,$post_state,$post_zip_code,$post_country,$post_language,$post_notes))
{
    die('<center><b>Error:</b> Failed to create client account!</center>');
}
  
    // Show box on clients page
    header("Location: clients.php?info=created");
    exit;
}
