...
{
    die('<center><b>Error:</b> You must delete or rename the <b>install</b> directory before you can log in.</center>');
}
...
// Page Title
$smarty->assign('pagetitle', 'Admin Login');
...
elseif(isset($_POST['login']))
{
    ...
    $result_check = @mysql_query("SELECT COUNT(id) AS logincount FROM admins WHERE username='$post_username' AND password=md5('$post_password')") or die('<center><b>Error:</b> Failed to query admins table!</center>');
    ...
    if($count_login >= 1)
    {
        ...
        //
        $result_status = @mysql_query("SELECT status FROM admins WHERE username='$post_username' AND password=md5('$post_password')") or die('<center><b>Error:</b> Failed to query admins table for status!</center>');
        ...
        $result_last = @mysql_query("SELECT id,orig_ip,last_login,last_ip,last_host FROM admins WHERE username='$post_username' AND password=md5('$post_password')") or die('<center><b>Error:</b> Failed to query admins table for last login!</center>');
        ...
        if(empty($orig_ip))
        {
            @mysql_query("UPDATE admins SET logged_in = 'Y',session_id = '$this_sessid',orig_login=NOW(),orig_ip='$user_ip',orig_host='$user_host',last_login=NOW(),last_ip='$user_ip',last_host='$user_host' WHERE username='$post_username' AND password=md5('$post_password')") or die('<center><b>Error:</b> Failed to update the admins table!</center>');
        }
        // They've been here before; just update last logins
        else
        {
            @mysql_query("UPDATE admins SET logged_in = 'Y',session_id = '$this_sessid',last_login=NOW(),last_ip='$user_ip',last_host='$user_host' WHERE username='$post_username' AND password=md5('$post_password')") or die('<center><b>Error:</b> Failed to update the admins table!</center>');
        }
